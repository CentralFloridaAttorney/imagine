...
class SheetMaker:
    def __init__(self, _image_file_path=IMAGE_FILE_PATH_1):
        ...
        self.resized_image = self.image.resize(
            size=(int(self.item_scale * self.width), int(self.item_scale * self.height)))
        # self.sized_image = self.image.resize((scaled_width, scaled_width))
        print("SheetMaker.__init__ done!")

    def add_resized_images(self):
        image_1 = Image.open(IMAGE_FILE_PATH_1).convert("RGBA").resize(
            size=(int(self.item_scale * self.width), int(self.item_scale * self.height)))
        image_2 = Image.open(IMAGE_FILE_PATH_2).convert("RGBA").resize(
            size=(int(self.item_scale * self.width), int(self.item_scale * self.height)))
        image_3 = Image.open(IMAGE_FILE_PATH_3).convert("RGBA").resize(
            size=(int(self.item_scale * self.width), int(self.item_scale * self.height)))
        image_4 = Image.open(IMAGE_FILE_PATH_4).convert("RGBA").resize(
            size=(int(self.item_scale * self.width), int(self.item_scale * self.height)))
        image_5 = Image.open(IMAGE_FILE_PATH_5).convert("RGBA").resize(
            size=(int(self.item_scale * self.width), int(self.item_scale * self.height)))
    ...
    def make_coin_sheet(self):
        ...
        item_image = item_image.resize(
            size=(int(image_width * image_width_scale), int(image_height * image_height_scale)))
